...
async function writeAuditLog(
  client,
  { violation_id, action, performed_by, details },
) {
  ...
  const previous_hash =
    lastEntry.rows.length > 0 ? lastEntry.rows[0].entry_hash : "0".repeat(64);

  const created_at = new Date().toISOString();
  const entry_hash = computeHash({
    previous_hash,
    violation_id,
    action,
    performed_by,
