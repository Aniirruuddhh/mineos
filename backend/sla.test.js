...
test("resolved and closed violations are never escalated", () => {
  ...
  assert.deepEqual(
    getEscalationStatus({ category: "safety", status: "resolved", created_at }),
    { escalation_status: "closed", hours_remaining: null },
  );
  assert.deepEqual(
    getEscalationStatus({ category: "safety", status: "closed", created_at }),
    { escalation_status: "closed", hours_remaining: null },
  );
});
