...
type ApiViolation = {
  ...
  evidence: { name: string } | null; latitude: number | null; longitude: number | null; created_at: string
}
...
function initials(name: string) { return name.split(' ').map((word) => word[0]).filter(Boolean).slice(0, 2).join('').toUpperCase() }
...
function needsAttention(violation: ApiViolation) {
  return violation.escalation_status === 'escalated' || (
...
export default function Page() {
  ...
  const filtered = useMemo(() => {
    ...
    return violations.filter((violation) => {
      if (date === 'Today' && new Date(violation.created_at) < startOfToday) return false
      if (date === 'Current workload' && ['resolved', 'closed'].includes(violation.status)) return false
      if (tab === 'Open') return !['resolved', 'closed'].includes(violation.status)
      if (tab === 'Due soon') return violation.escalation_status === 'on_track' && violation.hours_remaining !== null && violation.hours_remaining <= 24
